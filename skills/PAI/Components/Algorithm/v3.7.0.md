...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "MESSAGE", "voice_id": "{DAIDENTITY.VOICEID}", "voice_enabled": true}'
```
...
**Voice (FIRST action after loading this file):** `curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Entering the Algorithm", "voice_id": "{DAIDENTITY.VOICEID}", "voice_enabled": true}'`
...
- **Never assert without verification (CRITICAL).** NEVER tell {PRINCIPAL.NAME} something "is" a certain way unless you have verified it with your own tools. After making changes, verify the result before claiming success. Evidence required — tests, screenshots, diffs. Never "Done!" without proof.
