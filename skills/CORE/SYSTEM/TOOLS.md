...
## FileHasher.ts - SHA-256 Content-Based Change Detection

**Location:** `~/.claude/skills/CORE/Tools/FileHasher.ts`

SHA-256 content hashing utility for detecting actual file content changes (not just metadata/timestamp changes). Based on ClawdBot/Moltbot memory system research which uses content hashing for reliable change detection.

**Usage:**
```bash
# Hash a single file (outputs just the hash)
bun ~/.claude/skills/CORE/Tools/FileHasher.ts /path/to/file.txt

# Hash with JSON output (includes metadata)
bun ~/.claude/skills/CORE/Tools/FileHasher.ts --json /path/to/file.txt

# Hash multiple files
bun ~/.claude/skills/CORE/Tools/FileHasher.ts --json file1.txt file2.txt

# Hash a string directly
bun ~/.claude/skills/CORE/Tools/FileHasher.ts --content "Hello, World!"

# Check if file changed from known hash (exit 0=unchanged, 1=changed)
bun ~/.claude/skills/CORE/Tools/FileHasher.ts --check <previous_hash> /path/to/file.txt
```

**Programmatic Usage:**
```typescript
import { hashFile, hashContent, detectChanges, saveHashMetadata, loadHashMetadata } from './FileHasher';
...
// Detect changes
const changes = await detectChanges('/path/to/file.txt', previousHash);
if (changes.changed) {
  console.log('File modified!');
}

// Store/retrieve hash metadata
await saveHashMetadata('/path/to/file.txt', result, '/path/to/metadata/');
const metadata = await loadHashMetadata('/path/to/file.txt', '/path/to/metadata/');
```
...
- Memory system file synchronization

**Technical Details:**
- Uses Node.js crypto module (no external dependencies)
- Streaming support for large files (avoids memory issues)
- 64-character hexadecimal output (SHA-256)
- Includes metadata: path, size, hashedAt timestamp
- Known hash of empty string: `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`

**Performance:**
- Small files: Read entire file into memory (faster)
- Large files: Use `{ streaming: true }` option for chunked processing
- 5MB file hashes in under 5 seconds
...
### Memory System
- Content-based change detection: `FileHasher.ts`
